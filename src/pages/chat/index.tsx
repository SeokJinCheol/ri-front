...
function Sources({ sources }: { sources: Citation[] }) {
    ...
    return <div className="mt-4 space-y-2">{sources.map((source) => <details key={`${source.document_id}:${source.chunk_index}`} className="rounded-lg border bg-background p-3">
        <summary className="cursor-pointer text-sm"><span className="font-medium">[{source.number}] {source.filename}</span><span className="ml-2 text-muted-foreground">{t("pages.chat.index.001", { v0: source.chunk_index + 1 })}</span></summary>
        {source.available ? <><p className="mt-3 whitespace-pre-wrap break-words text-sm leading-relaxed">{source.content}</p><Button asChild variant="link" className="px-0"><Link to={`/documents/${source.document_id}`}><FileText />{t("pages.chat.index.002")}</Link></Button></> : <p className="mt-3 text-sm text-muted-foreground">{t("pages.chat.index.003")}</p>}
    </details>)}</div>
}
...
function ServiceChat({ projectId, serviceId, email, generationReady }: { projectId: string; serviceId: string; email: string; generationReady: boolean }) {
    ...
    return <div className="grid gap-5 lg:grid-cols-[240px_minmax(0,1fr)]">
        <aside className="space-y-3 rounded-xl border bg-card p-4">
            <Button variant="outline" className="w-full" disabled={busy} onClick={newChat}><Plus />{t("pages.chat.index.004")}</Button>
            <h2 className="pt-2 text-sm font-semibold">{t("pages.chat.index.005")}</h2>
            {loading && <p role="status" className="text-xs text-muted-foreground">{t("pages.chat.index.006")}</p>}
            {!loading && !conversations.length && <p className="text-sm text-muted-foreground">{t("pages.chat.index.007")}</p>}
            {conversations.map((item) => <div key={item.id} className={`flex items-center rounded-md ${active?.id === item.id ? 'bg-muted' : ''}`}>
                ...
                <Button variant="ghost" size="icon" disabled={busy} aria-label={t("pages.chat.index.008", { v0: item.title })} onClick={async () => {
                    if (locked.current || !window.confirm(t("pages.chat.index.009"))) return
        ...
        <div className="min-w-0 space-y-5">
            <fieldset disabled={busy} className="rounded-xl border bg-card p-4">
                <legend className="px-1 text-sm font-semibold">{t("pages.chat.index.010")}</legend>
                <p className="mb-3 text-xs text-muted-foreground">{t("pages.chat.index.011")}</p>
                {indicesLoading && <p role="status">{t("pages.chat.index.012")}</p>}
                {indicesError && <p role="alert" className="text-destructive">{indicesError}</p>}
                {!indicesLoading && !available.length && <p className="text-sm text-muted-foreground">{t("pages.chat.index.013")}<Link className="underline" to="/index">{t("pages.chat.index.014")}</Link></p>}
                <div className="flex flex-wrap gap-2">{available.map((index) => <label key={index.id} className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm ${selected.includes(index.id) ? 'border-primary bg-primary/5' : ''}`}>
                    ...
                    }} />{index.name}<span className="text-muted-foreground">{t("pages.chat.index.015", { v0: index.document_count })}</span>
                </label>)}</div>
            </fieldset>
            {!generationReady && <p className="rounded-lg bg-muted p-4 text-sm">{t('chat.configureModel')} <Link className="underline" to={`/service/${serviceId}/edit`}>{t("pages.chat.index.018")}</Link></p>}
            <div className="min-h-60 space-y-5 rounded-xl border bg-card p-5" aria-live="polite">
                {!messages.length && !busy && <div className="py-10 text-center"><MessageSquare className="mx-auto mb-3 size-8 text-muted-foreground" /><h2 className="font-medium">{t("pages.chat.index.019")}</h2><p className="mt-2 text-sm text-muted-foreground">{t("pages.chat.index.020")}</p></div>}
                ...
                {busy && <div role="status" className="space-y-2 text-sm"><p className="whitespace-pre-wrap break-words">{pendingQuestion}</p><p className="animate-pulse text-muted-foreground">{t("pages.chat.index.021")}</p></div>}
            </div>
            {sources !== null && <section className="rounded-xl border p-5"><h2 className="font-semibold">{t("pages.chat.index.022", { v0: sources.length })}</h2><p className="mt-1 text-xs text-muted-foreground">{t("pages.chat.index.023")}</p><Sources sources={sources} />{!sources.length && <p className="mt-3 text-sm">{t("pages.chat.index.024")}</p>}</section>}
            {error && <div role="alert" className="space-y-2 rounded-lg border border-destructive/30 p-4 text-sm"><p className="text-destructive">{error}</p>{failed && <Button variant="outline" disabled={busy} onClick={() => void submit(undefined, true)}>{t("pages.chat.index.025")}</Button>}</div>}
            <form onSubmit={(event) => void submit(event)} className="space-y-3">
                <label htmlFor="chat-question" className="text-sm font-medium">{t("pages.chat.index.026")}</label>
                <textarea id="chat-question" className="min-h-28 w-full resize-y rounded-xl border bg-background p-4 text-sm" value={question} onChange={(e) => setQuestion(e.target.value)} maxLength={4000} disabled={busy} required placeholder={t("pages.chat.index.027")} />
                <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-muted-foreground">{t("pages.chat.index.028", { v0: question.length })}</p><div className="flex gap-2">
                    <Button type="button" variant="outline" disabled={busy || !selected.length || !question.trim()} onClick={() => void submit(undefined, false, true)}><Search />{t("pages.chat.index.029")}</Button>
                    <Button type="submit" disabled={busy || !selected.length || !question.trim() || !generationReady}><Send />{t("pages.chat.index.030")}</Button>
                </div></div>
            </form>
        </div>
    </div>
...
function ProjectChat({ projectId, email }: { projectId: string; email: string }) {
    ...
    return <section className="space-y-6 p-3">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-2xl font-semibold">{t('nav.chat')}</h1><p className="mt-2 text-sm text-muted-foreground">{t("pages.chat.index.031")}</p></div>
            {!!services.length && <div className="w-full space-y-2 sm:w-64"><label htmlFor="chat-service" className="text-sm font-medium">{t("pages.chat.index.032")}</label><Select id="chat-service" value={selected?.id ?? ''} onValueChange={setServiceId}>{services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</Select></div>}
        </div>
        {loading && <p role="status">{t("pages.chat.index.033")}</p>}
        {error && <div role="alert"><p className="text-destructive">{error}</p><Button variant="outline" onClick={reload}>{t("routes.index.001")}</Button></div>}
        {!loading && !error && !selected && <div className="rounded-xl border border-dashed p-10 text-center"><p>{t("pages.chat.index.034")}</p><Button asChild className="mt-4"><Link to="/service/new">{t("pages.chat.index.035")}</Link></Button></div>}
